"""
En samling hjelpefunksjoner som bruker nvdbapiv4-funksjonene til å gjøre nyttige ting, f.eks. lagre geografiske datasett
...
import nvdbapiv4
from nvdbapiv4 import apiforbindelse
...
def records2gpkg( minliste, filnavn, lagnavn ): 
    """
    Tar en liste med records (dictionaries) a la dem vi får fra nvdbapiv4.to_records() og skriver til geopackage
...
def nvdb2gpkg( objekttyper, filnavn='datadump', mittfilter=None, vegnett=True, vegsegmenter=False, geometri=True, forb=None):
    ...
    KEYWORDS
        mittfilter=None : Dictionary med filter til søkeobjekt i nvdbapiv4.py, for eksempel { 'kommune' : 5001 }
        ...
        forb=None eller en instans av nvdbapiv4.apiforbindelse(). Dette objektet håndterer innlogging i NVDB api LES, som er 
    ...
    for enObjTypeId in objekttyper: 
        ...
        sok = nvdbapiv4.nvdbFagdata( enObjTypeId  )
        ...
        lagnavn = 'type' + str(enObjTypeId) + '_' + nvdbapiv4.esriSikkerTekst( objtypenavn.lower() ) 
...
def dumpkontraktsomr( komr = [] ): 
    ...
    for enkontrakt in komr: 
        ...
        filnavn = nvdbapiv4.esriSikkerTekst( enkontrakt )
...
def firefeltrapport( mittfilter={}): 
    ...
    Bruker søkeobjektet nvdbapiv4.nvdbVegnett fra biblioteket https://github.com/LtGlahn/nvdbapiv4
...
def posisjon( params, forb=None ): 
    ...
    KEYWORDS: 
        forb : None eller en instans av nvdbapiv4.apiforbindelse()
...
def kostraFiltrerDF( minDataFrame ): 
    ...
    Input data kan også være NVDB fagdata med vegsegmenter, typisk laget med nvdbapiv4.nvdbFagData().to_records(vegsegmenter=True)
...
def nvdbsok2geojson( sokeobjekt, filnavn, mittfilter=None, srid=4326, **kwargs ): 
    ...
    ARGUMENTS
        sokeobjekt : NVDB søkeobjekt fra nvdbapiv4.nvdbFagdata( <NVDB objektType.ID>) eller nvdbapiv4.nvdbVegnett()
